...
def _assign_equal_freq_bins(series: pd.Series, n_bins: int = 10, ascending: bool = True) -> pd.Series:
...
def decile_analysis_plot(
    ...
    n_bins: int = 10,
    ascending: bool = True,
    bar_color: str = "#4527a0",  # purple  # TODO(use COLOR_PRIMARY)
    line_color: str = "#fa6300",  # orange  # TODO(use COLOR_PRIMARY)
    ...
    fig.add_hline(
        y=mean_default_rate,
        line_dash="dash",
        line_color="#059fed",  # TODO(use COLOR_PRIMARY)
        secondary_y=True,
        annotation_text="<b>Média</b>",
        annotation_font_color="#059fed",  # TODO(use COLOR_PRIMARY)
    )
...
def _demo() -> None:  # pragma: no cover – only for visual test
    ...
    fig, ks_value, table = decile_analysis_plot(df_demo, score_col="score", target_col="bad_flag", ascending=False)
