...
def _evaluator(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("riskpilot.evaluation.binary_performance_evaluator.shap", object())
    return bev
...
def test_cache_hit(tmp_path, monkeypatch):
    ...
    t0 = time.time()
    bev._compute_shap_values(bev.df_train[bev.predictor_cols], split_name="train", use_cache=True)
    t1 = time.time() - t0
    t0 = time.time()
    bev._compute_shap_values(bev.df_train[bev.predictor_cols], split_name="train", use_cache=True)
    t2 = time.time() - t0
...
def test_disk_cache(tmp_path, monkeypatch):
    ...
    bev1._compute_shap_values(bev1.df_train[bev1.predictor_cols], split_name="train", use_cache=True)
    bev2 = _evaluator(tmp_path, monkeypatch)
    bev2._compute_shap_values(bev2.df_train[bev2.predictor_cols], split_name="train", use_cache=True)
